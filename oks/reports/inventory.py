...
class InventoryReport(Report):
    ...
    OPTIONS = [
        ("common_items", oks.REPORT_OPTION_BOOL, "Incluir itens comuns", True),
        (
            "raw_materials",
        ...
        (
            "production_items",
            oks.REPORT_OPTION_BOOL,
            "Incluir itens de produção",
            True,
        ),
        ("services", oks.REPORT_OPTION_BOOL, "Incluir serviços", True),
        ("name", oks.REPORT_OPTION_TEXT, "Filtrar por nome", ""),
    ]
    ...
    def make(self):
        ...
        items = []
        for option, type_ in (
            (self.common_items, oks.TYPE_COMMON_ITEM),
            ...
            if option:
                items.extend(
                    self.db.cached_load(
                        oks.ITEM, type_=type_, name="~%%%s%%" % self.name
                    )
                )
        ...
        total = 0.0
        for item in items:
            self.data.append((item.name, item.quantity, item.price))
