...
class OperationsReport(Report):
    ...
    def make(self, type_):
        ...
        operations = self.db.cached_load(
            oks.OPERATION,
        ...
        for operation in operations:
            ...
            total += subtotal
            self.data.append(
                [operation.date, operation.company, operation.oid, subtotal]
            )
        if total:
    ...
    def make_output(self):
        ...
        title = "{0} - {1} a {2}".format(
            self.NAME, date_to_str(self.start_date), date_to_str(self.end_date)
        )
        return TableSection(
            "compact_companies_report",
            title,
            (
                DateData("date", "Data"),
...
class ProductionOperationsReport(Report):
    ...
    def make(self):
        ...
        if count != 0:
            self.bits.append(
                "%i itens em %i operações" % (len(self.data), count)
            )
...
class ProductionCostReport(Report):
    ...
    DESCRIPTION = (
        "Listar os custos da produção de itens em operações de um "
        "dado período."
    )
    ...
    def make(self):
        ...
        raw_materials = [
            ...
            for item in self.db.cached_load(
                oks.ITEM, type_=oks.TYPE_RAW_MATERIAL
            )
        ]
        ...
        self.bits.append(
            CurrencyData("total", "Custo total no período", total, unit="R$")
        )
