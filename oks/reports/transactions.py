...
class TransactionsReport(Report):
    ...
    def make(self, type_):
        ...
        transactions = self.db.cached_load(
            oks.TRANSACTION, IO=type_, status=status
        )
        total = 0.0
        operations = {}
        for transaction in transactions:
            ...
            if pid not in operations:
                operations[pid] = self.db.cached_load(
                    oks.OPERATION, row_id=pid
                )
