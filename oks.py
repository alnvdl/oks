...
class DialogTable(Window):
    ...
    def save_state(self):
        if self.mode == oks.MODE_MAIN:
            ...
            self.tables[self.table]["main_state"] = (True, path, scroll)
    ...
    def load_table(self, table, force_reload=False):
        ...
        # Treeview
        if table not in list(self.models.keys()):
        ...
        for column in self.tables[self.table]["columns"]:
            factory, args = column[0], column[1:]
            self.treeview.append_column(factory(*args))
    ...
    def clear_report_actions(self):
        for child in self.table_report_options.get_children():
            self.table_report_options.remove(child)
    ...
    def visible_func(self, model, iter_, data):
    ...
    def get_selected_row_id(self):
        ...
        if rowIter is not None:
            if self.SORT:
                rowIter = self.sf_model.convert_iter_to_child_iter(rowIter)
    ...
    def get_row_id_from_path(self, path):
        if self.SORT:
            path = self.sf_model.convert_path_to_child_path(gtk.TreePath(path))
        path = self.f_model.convert_path_to_child_path(gtk.TreePath(path))
...
if __name__ == '__main__':
    ...
    mainWindow = DialogTable(gladeFile, styleFile, db_path)
    mainWindow.run()
